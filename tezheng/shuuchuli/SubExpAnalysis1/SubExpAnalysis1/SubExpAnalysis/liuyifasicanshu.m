...
%      X(yanzheng) =  xielva*TQPY(yanzheng)+xielvb;
    ...
    [plcc, mae, rms, srcc, krcc] = IQA_eval(MOSY, FMOS', 'regress_logistic');
    PLCC(liuyifashiyan) = plcc;
    SRCC(liuyifashiyan) = srcc;
    MAE(liuyifashiyan) = mae;
    RMSE(liuyifashiyan) = rms;
    KRCC(liuyifashiyan) = krcc;
